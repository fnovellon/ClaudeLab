...
**Post-game stats are Daily-mode only.** `endGame()` calls `recordDailyResult(won, guessCount)` only when `mode === "daily"`, cumulating games-played/win-rate/current-and-max-streak/guess-distribution into a single global `localStorage` key (`bleachdle-stats`, separate from the per-date daily-state key). `renderStats()` shows the `#statsPanel` section only when `mode === "daily" && state.finished`; it's hidden otherwise, including for the entire Infinite mode (consistent with Infinite mode never touching `localStorage` — see above). Streaks are day-based: `recordDailyResult` compares the previous `lastResultDate` to the current UTC date via `isNextUTCDay()` and only extends the streak on a win recorded exactly one UTC day after the last one; a loss always resets `currentStreak` to 0.
